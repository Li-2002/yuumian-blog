...
export default defineAppSetup(({ isClient }) => {
  ...
  const maxRetry = 50
  const injectFooter = () => {
    const footer = document.querySelector('footer.yun-footer')
    if (!footer) {
    ...
    // 防止重复注入
    if (document.getElementById('footer-link-bar'))
      return

    const bar = document.createElement('div')
    bar.id = 'footer-link-bar'
    bar.innerHTML = `
