...
直到问题越来越复杂，我们需要开始用到<span style="color: rgba(66, 133, 244, 1)">**非线性函数**</span>了，怎么变成非线性呢，很简单，在原来的函数基础上套一个非线性的运算即可比如<code>f(x)=(wx+b)²</code>、<code>f(x)=sin(wx+b)</code>等等这些都是，即是<span style="color: rgba(66, 133, 244, 1)">**f(x)=g(wx+b) 激活函数**</span>，而且我们实际上不会只有一个x输入，可能会有很多个输入，所以可能会有多个x，每个x会对应一个w 即是
![](https://yuumii.top/article/20260812_LearningSpringAI2/1786506574249-b46d9709-7e55-4f59-be3e-0fa7762a426a.png)

甚至有的时候一层激活函数还不足以让大部分点都在线上，也就是说这条线还不够灵活 或者不够弯，我们还需要再<span style="color: rgba(66, 133, 244, 1)">**套一层激活函数**</span>，理论上可以无限的去套娃，来构建一条任意无限灵活的函数
